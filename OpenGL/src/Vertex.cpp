...
namespace Vertex {
	std::vector<Vertex> GenerateRectVertex(float x_length, float y_length, float n) {
	...
	std::vector<unsigned int> CalculateIndices(std::vector<Vertex> vertices, int n) {
	...
	void CalculateNormals(std::vector<Vertex>& vertex, const std::vector<unsigned int>& indices, float n) {
	...
	std::vector<unsigned int> CalculateTorusIndices(std::vector<Vertex> vertices, int n) { //Under Construction
		std::cout << "Calculating Indices " << std::endl;
		std::vector<unsigned int> Indices;
		int size = pow(n, 2) + n - 1;
